...
const CrowdloanMemoUpdatedEventId = '0x4908';
...
//   const upwardMessagesReceivedEvents = block.events.filter(e => e.event.section === 'parasUmp' && e.event.method === 'UpwardMessagesReceived') as SubstrateEvent[];
...
//     const crowdloanEvents = event.extrinsic.events.filter(e => (e.event.index as EventId).toString() == CrowdloanContributedEventId || CrowdloanMemoUpdatedEventId) as SubstrateEvent[];
//     const len = crowdloanEvents.length;
//     for (let i = 0; i < len; i++) {
//       if ((crowdloanEvents[i].event.index as EventId).toString() == CrowdloanContributedEventId && i < len - 1) {
//         if ((crowdloanEvents[i + 1].event.index as EventId).toString() == CrowdloanMemoUpdatedEventId) {
//           const record = new CrowdloanMerged(blockNumber.toString() + '-' + crowdloanEvents[i].idx.toString());
//           record.block_height = blockNumber;
...
//           record.account = crowdloanEvents[i].event.data[0].toString();
//           record.para_id = crowdloanEvents[i].event.data[1].toString();
//           record.balance = crowdloanEvents[i].event.data[2].toString();
//           record.memo = crowdloanEvents[i + 1].event.data[2].toString();
//           await record.save();
//         }
//       } else if ((crowdloanEvents[i].event.index as EventId).toString() == CrowdloanContributedEventId && i == len - 1) {
//         logger.info(`This Contributed event missing matching MemoUpdated event:${blockNumber}-${crowdloanEvents[i].idx}`)
//       }
//     }
//   }
// }
