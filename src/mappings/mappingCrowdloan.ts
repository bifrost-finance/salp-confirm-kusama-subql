...
import { Memoupdated } from "../types/models/Memoupdated";
import { Contributed } from "../types/models/Contributed";
...
export async function handleMemo(event: SubstrateEvent): Promise<void> {
  ...
  const entity = new Memoupdated(blockNumber.toString() + '-' + event.idx.toString());
